...
namespace DTcms.Common
{
    public class Utils
    {
        ...
        /// <param name="centSize">中间页码数量</param>
        /// <returns></returns>
        public static string OutPageListUser(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            //计算页数
            if (totalCount < 1 || pageSize < 1)
            {
            ...
            if (pageIndex == 1)
            {
                firstStr = "<a class=\"active\">1</a>";
            }
            if (pageIndex == pageCount)
            {
                lastStr = "<a class=\"active\">" + pageCount.ToString() + "</a>";
            }
            int firstNum = pageIndex - (centSize / 2); //中间开始的页码
            if (pageIndex < centSize)
                firstNum = 2;
            ...
            for (int i = firstNum; i <= lastNum; i++)
            {
                if (i == pageIndex)
                {
                    pageStr.Append("<a class=\"active\">" + i + "</a>");
                }
                else
                {
                    pageStr.Append("<a href=\"" + ReplaceStr(linkUrl, pageId, i.ToString()) + "\">" + i + "</a>");
