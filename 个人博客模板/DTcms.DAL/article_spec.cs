...
namespace DTcms.DAL
{
	...
	public partial class article_spec
	{
        ...
        {
            string strSql = "select top 1 id from " + databaseprefix + "article_spec order by id desc";
        ...
        /// 查找不存在的规格值并删除
        /// </summary>
        private void DeleteList(OleDbConnection conn, OleDbTransaction trans, List<Model.article_spec_value> models, int parentId)
        {
            ...
            if (models != null)
            {
                ...
                {
                    DbHelperOleDb.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_spec where parent_id=" + parentId + " and id not in(" + id_list + ")");
                }
            }
        }
        #endregion
