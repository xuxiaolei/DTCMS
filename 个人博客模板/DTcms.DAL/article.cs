                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        OleDbParameter[] parameters = {
					            ...
					            new OleDbParameter("@tags", OleDbType.VarChar,500),
                        ...
                        if (model.specs != null)
                        {
                            ...
                            foreach (Model.article_goods_spec modelt in model.specs)
                            {
                                ...
                                OleDbParameter[] parameters6 = {
					                    new OleDbParameter("@article_id", OleDbType.Integer,4),
					                    new OleDbParameter("@spec_id", OleDbType.Integer,4),
                                ...
                                if (modelt.img_url != null)
                                {
                                    parameters6[4].Value = modelt.img_url;
                                }
                                else
                                {
                                    parameters6[4].Value = string.Empty;
                                }
                                DbHelperOleDb.ExecuteSql(conn, trans, strSql6.ToString(), parameters6); //带事务
                            }
                        }
                        #endregion
            ...
            using (OleDbConnection conn = new OleDbConnection(DbHelperOleDb.connectionString))
            {
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        #region 修改商品规格==========================
                        //先删除旧的规格
                        DbHelperOleDb.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_goods_spec  where article_id=" + model.id);
                        if (model.specs != null)
                        {
                            StringBuilder strSql6;
                            foreach (Model.article_goods_spec modelt in model.specs)
