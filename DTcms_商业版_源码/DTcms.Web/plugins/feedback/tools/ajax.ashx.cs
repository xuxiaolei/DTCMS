...
namespace DTcms.Web.Plugin.Feedback.tools
{
    /// <summary>
    /// 管理后台AJAX处理页
    /// </summary>
    public class ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        Model.feedbackconfig config = new BLL.feedbackconfig().loadConfig("../config/install.config");
        ...
        private void feedback_list(HttpContext context)
        {
            ...
            int totalcount = 0;
            DataTable dt = new BLL.feedback().GetList(pageSize, page, "is_lock=0 and site_path='" + site_path + "'", "add_time desc", out totalcount).Tables[0];
