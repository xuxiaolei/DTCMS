...
namespace DTcms.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "manager_validate": //验证管理员用户名是否重复
                    ...
                    break;
                case "tags_validate": //验证Tags调用名是否重复
                    tags_validate(context);
                    break;
                ...
                case "get_builder_html": //生成静态页面
                    ...
                    break; 
                case "get_category_list":  //类别列表
                    get_category_list(context);
                    break;
                case "get_content_list":  //列表
                    get_content_list(context);
                    break;
            }
        }
        ...
        private void tags_validate(HttpContext context)
        {
            string call_index = DTRequest.GetString("param");
            string old_name = DTRequest.GetString("old_name");
            if (string.IsNullOrEmpty(call_index))
            {
                context.Response.Write("{ \"info\":\"该名称不可为空！\", \"status\":\"n\" }");
                return;
            }
            if (call_index.ToLower() == old_name.ToLower())
            {
                context.Response.Write("{ \"info\":\"该名称可以使用\", \"status\":\"y\" }");
                return;
            }
        ...
        public void get_category_list(HttpContext context)
        {
            object json = null;
        ...
        /// </summary>
        /// <param name="context"></param>
        public void get_content_list(HttpContext context)
        {
            object json = null;
            int totalCount = 0;
