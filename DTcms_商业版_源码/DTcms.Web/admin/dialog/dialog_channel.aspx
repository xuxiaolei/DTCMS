...
<script type="text/javascript">
    ...
    $(function () {
        ...
        //缓存
        $("#rblCache").click(function () {
            if ($(this).is(':checked')) {
                $("#cachepath").show();
                api.height(parseInt(top.$("iframe[name='dialogChannel']").height()) + 40);
            } else {
                $("#cachepath").hide();
                api.height(parseInt(top.$("iframe[name='dialogChannel']").height()) - 40);
            }
        ...
        if ($(api.data).length > 0) {
            ...
            if (parentObj.find("input[name='item_cache']").val() != "") {  //是否开启缓存
                $("#rblCache").attr("checked", true);
                $("#cachepath").show();
            }
            $("#cachePath").val(parentObj.find("input[name='item_path']").val()); //缓存路径
    ...
    function getTrHtml() {
        ...
        + '<td><input type="hidden" name="item_cache" /><input type="hidden" name="item_path" /><span class="item_cache"></span></td>'
    ...
    function setNavRow(obj) {
        ...
        //模版缓存
        if ($("#rblCache").is(':checked')) {
            $(obj).find("input[name='item_cache']").val('1');
            $(obj).find(".item_cache").html("是");
        } else {
            $(obj).find("input[name='item_cache']").val('');
            $(obj).find(".item_cache").html("-");
        }
        $(obj).find("input[name='item_path']").val($("#cachePath").val());
        //分页大小
...
<div class="div-content">
  ...
  <dl>
    <dt>开启缓存</dt>
    <dd>
        <input type="checkbox" name="rblCache" id="rblCache" />
    </dd>
  </dl>
  <dl id = "cachepath" style="display:none;">
    <dt>缓存名称</dt>
    <dd>
      <input type="text" id="cachePath" value="" class="input normal" />
      <span>*无扩展名，变量格式：{变量名}</span>
    </dd>
  </dl>
  <dl>
    <dt>URL表达式</dt>
