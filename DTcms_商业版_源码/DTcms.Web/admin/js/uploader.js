...
function addImage(targetObj, originalSrc, thumbSrc) {
    //排序
    var sort_id = $("#photo-list").find("li").length || 99;
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_name" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="hid_photo_remark" value="" />'
    + '<input type="hidden" name="hid_photo_sort" class="hid_photo_sort" value="' + (sort_id + 1) + '" />'
    ...
    + '<a href="javascript:;" onclick="ajaxLoadImg(this)">浏览</a>'
