    ...
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //编辑权限
                if (!ChkAuthority("channel_" + this.channel_name + "_list", this.action))
                {
            ...
            foreach (Model.article_attribute_field modelt in ls)
            {
                ...
                switch (modelt.control_type)
                {
                    ...
                    case "editor": //编辑器
                        ...
                        //创建一个Label控件
                        Label labelScript = new Label();
                        labelScript.CssClass = "script";
                        labelScript.Text = modelt.valid_tip_msg;
                        //创建Javascript
                        Label labelControl2a = new Label();
                        //是否简洁型编辑器
                        if (modelt.editor_type == 1)
                        {
                            txtTextArea.Attributes.Add("style", "height:150px; width:100%;");
                            labelScript.Text = "<script type=\"text/javascript\">var ue = UE.getEditor('" + "field_control_" + modelt.name + "',{toolbars:[['FullScreen', 'Source','bold','italic','underline','test','fontfamily','fontsize','forecolor','backcolor','insertimage','|','insertorderedlist','insertunorderedlist','indent','justifyleft','justifycenter','justifyright','removeformat','help']],elementPathEnabled:false,wordCount:false});</script>";
                        }
                        else
                        {
                            txtTextArea.Attributes.Add("style", "height:300px; width:100%;");
                            labelScript.Text = "<script type=\"text/javascript\">var ue = UE.getEditor('" + "field_control_" + modelt.name + "');</script>";
                        }
                        //将控件添加至DD中
                        htmlDD.Controls.Add(txtTextArea);
                        htmlDD.Controls.Add(labelScript);
                        htmlDD.Controls.Add(labelControl2a);
                        break;
                    case "images": //图片上传
                        goto case "single-text";
                    case "video": //视频上传
                    ...
                    case "similar": //同类选择
                        ...
                        labelControl6.Text = "<a class=\"icon-btn add\" href=\"javascript:;\" onclick=\"showChooseDialog(this,'field_control_" + modelt.name + "')\"><i></i><span>选择</span></a>";
                        //创建一个Table
                        Table table = new Table();
                        table.ID = "field_table_" + modelt.name;
                        table.Attributes.Add("cellpadding","0");
                        table.Attributes.Add("cellspacing","0");
                        table.Attributes.Add("class","listTable");
                        //创建一个DIV控件
                        HtmlGenericControl htmlDiv4 = new HtmlGenericControl("div");
                        htmlDiv4.Attributes.Add("class", "select_list");
                        htmlDiv4.Controls.Add(table);
                        //将控件添加至DD中
        ...
        private void ShowSysField(int _channel_id)
        {
            ...
            {
                attribute_tab_item.Visible = true;
                attribute_tab_content.Visible = true;
            }
            if (channelModel.is_comment == 1)
            {
        ...
        private void ShowInfo(int _id)
        {
            ...
            if (model.status == 2)
            {
                this.rblStatus.Items.Add(new ListItem("回收站", "2"));
            }
            //采集数据
            else if (model.status == 3)
            {
                this.rblStatus.Items.Add(new ListItem("采集数据","3"));
            }
            ...
            foreach (Model.article_attribute_field modelt1 in ls1)
            {
                switch (modelt1.control_type)
                {
                    ...
                    case "multi-checkbox": //多项多选
                        ...
                        break;
                    case "similar": //同类选择
                        Table table = FindControl("field_table_" + modelt1.name) as Table;
                        if (table != null && model.fields.ContainsKey(modelt1.name))
                        {
                            string value = model.fields[modelt1.name];
                            if (value != "")
                            {
                                DataTable dt = bll.GetList(20, string.Format("id in ({0})", value), "id desc").Tables[0];
                                foreach (DataRow dr in dt.Rows)
                                {
                                    TableRow trRow = new TableRow();
                                    trRow.Attributes.Add("class", "data");
                                    //ID
                                    TableCell tdId = new TableCell();
                                    tdId.Text = "<input type=\"hidden\" name=\"field_control_" + modelt1.name + "\" value=\"" + dr["id"].ToString() + "\">" + dr["id"].ToString();
                                    trRow.Controls.Add(tdId);
                                    //标题
                                    TableCell tdTitle = new TableCell();
                                    tdTitle.Text = dr["title"].ToString();
                                    trRow.Controls.Add(tdTitle);
                                    //删除
                                    TableCell tdDel = new TableCell();
                                    tdDel.Text = "<a href=\"javascript:;\" onclick=\"remoreThis(this)\">删除</a>";
                                    trRow.Controls.Add(tdDel);
                                    //填充
            ...
            //绑定自定义参数
            rptAttributeList.DataSource = model.attribute;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                switch (dr["control_type"].ToString())
                {
                    ...
                    case "multi-checkbox": //多项多选
                        ...
                        break;
                    case "similar": //同类选择
                        string[] idArr = Request.Form.GetValues("field_control_" + dr["name"]);
                        if (idArr != null && idArr.Length > 0)
                        {
                            ...
                            dic.Add(dr["name"].ToString(), string.Join(",", idList));
                        }
                        else
                        {
                            dic.Add(dr["name"].ToString(), dr["default_value"].ToString());
                        }
                        break;
                }
            }
        ...
        #region 获取编辑器内容第一张图片
        private string AutoGetFirstImage(string content)
        {
            if (string.IsNullOrEmpty(content))
            {
                return string.Empty;
            ...
            if (m.Success)
            {
                ...
                if (imgUri.StartsWith("http"))
                {
                    imgUri = new UI.UpLoad().remoteSaveAs(imgUri);
                }
                if (!string.IsNullOrEmpty(imgUri))
                {
                    //新文件名称
                    string newUrl = imgUri.Substring(0, imgUri.Length - imgUri.LastIndexOf(@"/") + 2) + Utils.GetRamCode() + Utils.GetFileExt(imgUri);
                    Thumbnail.MakeThumbnailImage(Utils.GetMapPath(imgUri), Utils.GetMapPath(newUrl), this.siteConfig.thumbnailwidth, this.siteConfig.thumbnailheight, this.siteConfig.thumbnailmode);
                    return newUrl;
                }
            }
            return string.Empty;
        ...
        private bool DoAdd()
        {
            ...
            else if (string.IsNullOrEmpty(model.img_url) && string.IsNullOrEmpty(txtImgUrl.Text.Trim()) && autoImg.Checked == true)
            {
                model.img_url = AutoGetFirstImage(txtContent.Value);
            }
            else
            {
            ...
            string[] sortArr = Request.Form.GetValues("hid_photo_sort");
            if (albumArr != null && albumArr.Length > 0)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        ...
                        {
                            ls.Add(new Model.article_albums { original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i], sort_id = sort_id });
                        }
                        else
                        {
                            ls.Add(new Model.article_albums { original_path = imgArr[1], thumb_path = imgArr[2], sort_id = sort_id });
                        }
                    }
                }
                model.albums = ls;
            ...
            #region 保存自定义参数==============
            //保存自定义参数
            string[] attributeNameArr = Request.Form.GetValues("hid_attribute_name");
            string[] attributeValueArr = Request.Form.GetValues("hid_attribute_value");
        ...
        private bool DoEdit(int _id)
        {
            ...
            Model.channel_site site = new BLL.channel_site().GetModel(model.site_id);
            ...
            if (albumArr != null)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        if (!string.IsNullOrEmpty(remarkArr[i]))
                        {
                            ls.Add(new Model.article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i], sort_id = sort_id });
                        }
                        else
                        {
                            ls.Add(new Model.article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2], sort_id = sort_id });
                        }
                    }
                }
                model.albums = ls;
        ...
        {
            string Url = this.backUrl != "" ? this.backUrl : "article_list.aspx?channel_id=" + this.channel_id;
