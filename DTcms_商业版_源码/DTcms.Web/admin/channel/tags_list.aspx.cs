    ...
    public partial class tags_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //添加权限
                if (!ChkAuthority("sys_article_tags", DTEnums.ActionEnum.Add.ToString()))
                {
                    this.addBtnPannel.Visible = false;
                }
                //修改权限
                if (!ChkAuthority("sys_article_tags", DTEnums.ActionEnum.Edit.ToString()))
                {
                    this.editBtnPannel.Visible = this.redBtnPannel.Visible = false;
                }
                //删除权限
                if (!ChkAuthority("sys_article_tags", DTEnums.ActionEnum.Delete.ToString()))
                {
