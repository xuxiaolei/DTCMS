    ...
    public partial class attribute_field_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //添加权限
                if (!ChkAuthority("sys_channel_field", DTEnums.ActionEnum.Add.ToString()))
                {
                    this.addBtnPannel.Visible = false;
                }
                //修改权限
                if (!ChkAuthority("sys_channel_field", DTEnums.ActionEnum.Edit.ToString()))
                {
                    this.editBtnPannel.Visible = false;
                }
                //删除权限
                if (!ChkAuthority("sys_channel_field", DTEnums.ActionEnum.Delete.ToString()))
                {
