...
<div class="tab-content">
  ...
  <dl>
    <dt>开启缓存</dt>
    <dd>
      <div class="rule-multi-radio" id="IsCache">
        <asp:RadioButtonList ID="rblCache" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow">
  ...
  <dl id = "cachepath">
    <dt>缓存名称</dt>
    <dd>
      <asp:TextBox ID="txtCachePath" runat="server" CssClass="input normal" />
      <span class="Validform_checktip">*无扩展名，变量格式：{变量名}</span>
    </dd>
  </dl>
  <script type="text/javascript">
      $(function () {
          $("#IsCache").find("a").on("click", function () { SignAnswer() });
      })
      function SignAnswer() {
          if ($("#rblCache input").eq(0).prop("checked") == true) {
              $("#cachepath").hide();
          } else {
              $("#cachepath").show();
          }
