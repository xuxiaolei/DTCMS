    ...
    public partial class plugin_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                //安装权限
                if (!ChkAuthority("sys_plugin_config", DTEnums.ActionEnum.Instal.ToString()))
                {
                    this.instalBtnPannel.Visible = false;
                }
                //卸载权限
                if (!ChkAuthority("sys_plugin_config", DTEnums.ActionEnum.UnLoad.ToString()))
                {
                    this.unLoadBtnPannel.Visible = false;
                }
                //生成权限
                if (!ChkAuthority("sys_plugin_config", DTEnums.ActionEnum.Build.ToString()))
                {
                    this.buildBtnPannel.Visible = false;
                }
