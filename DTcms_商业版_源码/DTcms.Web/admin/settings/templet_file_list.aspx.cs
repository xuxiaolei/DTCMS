...
namespace DTcms.Web.admin.settings
{
    public partial class templet_file_list : Web.UI.ManagePage
    {
        ...
        protected string filepath = string.Empty; //当前目录

        protected void Page_Load(object sender, EventArgs e)
        {
            skinName = DTRequest.GetQueryString("skin");
            filepath = DTRequest.GetQueryString("filepath");
        ...
        private void RptBind(string skin_name)
        {
            ...
            DirectoryInfo dirInfo = new DirectoryInfo(Utils.GetMapPath(@"../../templates/" + skin_name + "/" + filepath));
            //获取目录
            foreach (DirectoryInfo dir in dirInfo.GetDirectories())
            {
                DataRow dr = dt.NewRow();
                dr["type"] = "folder";
                dr["url"] = string.Format("templet_file_list.aspx?skin={0}&filepath={1}{2}", skinName, filepath, dir.Name);
                dr["target"] = "_self";
                dr["name"] = dir.Name;
            ...
            {
                if (file.Name != "about.xml" && file.Name != "preview.png")
                {
                    DataRow dr = dt.NewRow();
                    dr["type"] = "file";
                    switch (Utils.GetFileExt(file.Name))
                    {
                        ...
                        case ".html":
                            dr["type"] = "file";
                            dr["url"] = string.Format("templet_file_edit.aspx?path={0}&amp;filename={1}{2}", skinName, filepath, file.Name);
                        ...
                        case ".png":
                            dr["type"] = "img";
                            dr["url"] = string.Format("javascript:ballImages('../../templates/{0}/{1}{2}')", skin_name, filepath, file.Name);
                            dr["target"] = "_self";
                            break;
                        default:
                            dr["type"] = "file";
