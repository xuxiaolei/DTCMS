    ...
    public partial class nav_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                //添加权限
                if (!ChkAuthority("sys_navigation", DTEnums.ActionEnum.Add.ToString()))
                {
                    this.addBtnPannel.Visible = false;
                }
                //修改权限
                if (!ChkAuthority("sys_navigation", DTEnums.ActionEnum.Edit.ToString()))
                {
                    this.editBtnPannel.Visible = false;
                }
                //删除权限
                if (!ChkAuthority("sys_navigation", DTEnums.ActionEnum.Delete.ToString()))
                {
