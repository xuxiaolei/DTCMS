...
namespace DTcms.API.Weixin.Common
{
    ...
    public partial class MessageFunction
    {
        ...
        public IResponseMessageBase GetResponseMessageeMusic(RequestMessageText requestMessage, int ruleId)
        {
            ...
            if (model == null)
            {
                new BLL.weixin_response_content().Add(requestMessage.FromUserName, requestMessage.MsgType.ToString(), requestMessage.Content, "music", "-1", requestMessage.ToUserName);
            }
            else
            {
                ...
                new BLL.weixin_response_content().Add(requestMessage.FromUserName, requestMessage.MsgType.ToString(), requestMessage.Content, "music", "音乐链接：" + model.media_url + "|标题：" + model.title + "|描述：" + model.content, requestMessage.ToUserName);
