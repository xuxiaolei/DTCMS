...
namespace DTcms.BLL
{
	...
	public partial class channel_site
	{
        ...
        public bool Exists(string build_path)
        {
            ...
            //判断文件目录是否存在，不存在则自动创建
            Utils.DirectoryExists(siteConfig.webpath + "/" + DTKeys.DIRECTORY_REWRITE_ASPX + "/");
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.channel_site GetSiteModel(int channel_id)
        {
            int id = new BLL.channel().GetSiteId(channel_id);
            if (id > 0)
            {
                return dal.GetModel(id);
            }
