...
namespace DTcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        /// <returns></returns>
        public string queryurl(string _key, string _call_index, int _id)
        {
            if (string.IsNullOrEmpty(_call_index))
            {
                return linkurl(_key, _id);
            }
            return linkurl(_key, _call_index);
        }
        /// <summary>
        /// 返回URL重写统一链接地址
        /// </summary>
        ...
        /// <param name="_id">文章ID</param>
        /// <returns></returns>
        public string queryurl(string _key, string _call_index, string _id)
        {
            if (string.IsNullOrEmpty(_call_index) || "" == _call_index)
            {
                return linkurl(_key, _id);
            }
            //return "http://" + HttpContext.Current.Request.Url.Host + linkurl(_key, _call_index);
            return linkurl(_key, _call_index);
        }
        /// <summary>
        /// 返回URL重写统一链接地址
        ...
        /// <param name="_id">文章ID</param>
        /// <returns></returns>
        public string queryurl(string _key, string _url, string _call_index, string _id)
        {
            if (!string.IsNullOrEmpty(_url))
            {
                return _url;
            }
            else if (string.IsNullOrEmpty(_call_index) || "" == _call_index)
            {
                return linkurl(_key, _id);
            }
            return linkurl(_key, _call_index);
