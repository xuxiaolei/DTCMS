...
namespace DTcms.DAL
{
	...
	public partial class channel
	{
        ...
        public channel(string _databaseprefix)
        {
            ...
            this.column = "id,site_id,name,title,is_albums,is_attach,is_spec,is_attribute,is_comment,is_type,sort_id,seo_title,seo_keywords,seo_description";
        }
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        strSql.Append("site_id,name,title,is_albums,is_attach,is_spec,sort_id,seo_title,seo_keywords,seo_description,is_type,is_attribute,is_comment");
                        ...
                        new DAL.navigation(databaseprefix).Add(conn, trans, newNavId, "channel_" + model.name + "_recycle", "回收站", "article/recycle_list.aspx", 102, model.id, "Show,View,Edit,Delete,Audit");
                        trans.Commit();
                    }
                    catch
                    {
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        int newNavId = 0;
                        if (!new DAL.navigation(databaseprefix).Exists("channel_" + model.name))
                        {
                            //添加导航菜单
                            newNavId = new DAL.navigation(databaseprefix).Add(conn, trans, parent_id, "channel_" + model.name, model.title, "", model.sort_id, model.id, "Show");
                            new DAL.navigation(databaseprefix).Add(conn, trans, newNavId, "channel_" + model.name + "_list", "内容管理", "article/article_list.aspx", 99, model.id, "Show,View,Add,Edit,Delete,Audit");
                            new DAL.navigation(databaseprefix).Add(conn, trans, newNavId, "channel_" + model.name + "_category", "栏目类别", "article/category_list.aspx", 100, model.id, "Show,View,Add,Edit,Delete");
                            new DAL.navigation(databaseprefix).Add(conn, trans, newNavId, "channel_" + model.name + "_comment", "评论管理", "article/comment_list.aspx", 101, model.id, "Show,View,Delete,Audit,Reply");
                            new DAL.navigation(databaseprefix).Add(conn, trans, newNavId, "channel_" + model.name + "_recycle", "回收站", "article/recycle_list.aspx", 102, model.id, "Show,View,Edit,Delete,Audit");
                        }
                        else
                        {
                            ...
                            new DAL.navigation(databaseprefix).Update(conn, trans, "channel_" + oldModel.name + "_recycle", "channel_" + model.name + "_recycle"); //回收站
                        }
                        trans.Commit();
                    }
                    catch
        ...
        public Model.channel DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["is_attribute"].ToString() != null && row["is_attribute"].ToString() != "")
                {
                    model.is_attribute = int.Parse(row["is_attribute"].ToString());
                }
                if (row["is_comment"].ToString() != null && row["is_comment"].ToString() != "")
                {
