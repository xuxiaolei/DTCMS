...
namespace DTcms.DAL
{
    public partial class plugin
    {
        ...
        public plugin(string _cachefix, string _databaseprefix)
        {
            cachefix = _cachefix;
            ...
            foreach (XmlElement xe in xnList)
            {
                if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite")
                {
                        ...
                        && !string.IsNullOrEmpty(xe.Attributes["templet"].InnerText) && !string.IsNullOrEmpty(xe.Attributes["inherit"].InnerText))
                    {
                        ...
                        //生成模板文件
                        PageTemplate.GetTemplate(sitePath, tempPath, skinName, xe.Attributes["templet"].InnerText, xe.Attributes["page"].InnerText, xe.Attributes["inherit"].InnerText, DTKeys.DIRECTORY_REWRITE_PLUGIN, string.Empty, pagesizeStr, isCache, cachePath, cachefix, urlType, 0, 1);
                    }
                }
            }
        }
