    ...
    public class url_rewrite
    {
        ...
        public bool Add(Model.url_rewrite model)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.cache))
                    xe.SetAttribute("cache", model.cache);
                if (!string.IsNullOrEmpty(model.cachepath))
                    xe.SetAttribute("cachepath", model.cachepath);
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.Attributes["name"].Value.ToLower() == model.name.ToLower())
                    {
                        ...
                        if (!string.IsNullOrEmpty(model.cache))
                            xe.SetAttribute("cache", model.cache);
                        else if (xe.Attributes["cache"] != null)
                            xe.Attributes["cache"].RemoveAll();

                        if (!string.IsNullOrEmpty(model.cachepath))
                            xe.SetAttribute("cachepath", model.cachepath);
                        else if (xe.Attributes["cachepath"] != null)
                            xe.Attributes["cachepath"].RemoveAll();
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.Attributes["name"].Value.ToLower() == attrValue.ToLower())
                    {
                        ...
                        if (xe.Attributes["cache"] != null)
                            model.cache = xe.Attributes["cache"].Value;
                        if (xe.Attributes["cachepath"] != null)
                            model.cachepath = xe.Attributes["cachepath"].Value;
