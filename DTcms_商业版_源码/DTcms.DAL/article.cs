...
namespace DTcms.DAL
{
	...
	public partial class article
	{
                    ...
                    try
                    {
                        ...
                        if (model.albums != null)
                        {
                            ...
                            foreach (Model.article_albums modelt in model.albums)
                            {
                                ...
                                strSql3.Append("article_id,thumb_path,original_path,remark,sort_id)");
                                ...
                                parameters3[4].Value = modelt.sort_id;
                        ...
                        if (model.attribute != null)
                        {
                            ...
                            foreach (Model.article_attribute modelt in model.attribute)
                            {
                                strSql7 = new StringBuilder();
                                strSql7.Append("insert into " + databaseprefix + "article_attribute(");
                                strSql7.Append("article_id,title,value)");
                                ...
                                parameters7[1].Value = modelt.title;
                                parameters7[2].Value = modelt.value;
                        ...
                        {
                            string[] tagsArr = model.tags.Trim().Split(new string[] { ",", "，", " " }, StringSplitOptions.None);
                            if (tagsArr.Length > 0)
                            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.fields.Count > 0)
                        {
                            ...
                            if (new DAL.article_attribute_value(databaseprefix).Exists(model.id))
                            {
                                foreach (KeyValuePair<string, string> kvp in model.fields)
                                {
                        ...
                        #region 修改扩展参数==========================
                        //删除
                        DbHelperSQL.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_attribute where article_id=" + model.id.ToString());
                        // 添加/修改扩展参数
                        if (model.attribute != null)
                        {
                            StringBuilder strSql7;
		...
		public bool Delete(int id)
		{
            ...
            CommandInfo cmd = new CommandInfo("delete from " + databaseprefix + "article_attribute_value where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除图片相册
            cmd = new CommandInfo("delete from " + databaseprefix + "article_albums where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除附件
            cmd = new CommandInfo("delete from " + databaseprefix + "article_attach where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除用户组价格
            cmd = new CommandInfo("delete from " + databaseprefix + "user_group_price where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除商品价格;
            cmd = new CommandInfo("delete from " + databaseprefix + "article_goods where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除商品规格
            cmd = new CommandInfo("delete from " + databaseprefix + "article_goods_spec where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除扩展参数
            cmd = new CommandInfo("delete from " + databaseprefix + "article_attribute where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除Tags标签关系
            cmd = new CommandInfo("delete from " + databaseprefix + "article_tags_relation where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除评论
            cmd = new CommandInfo("delete from " + databaseprefix + "article_comment where article_id=@article_id", parameters);
            sqllist.Add(cmd);

            //删除主表
            cmd = new CommandInfo("delete from " + databaseprefix + "article where id=@article_id", parameters);
            sqllist.Add(cmd);
        ...
        public Model.article GetModel(int id)
        {
            ...
            DataSet ds = DbHelperSQL.Query("select " + this.column + " from " + databaseprefix + "article where id=@id", parameters);
        ...
        public Model.article GetModel(string call_index)
        {
            ...
            DataSet ds = DbHelperSQL.Query("select top 1 " + this.column + " from " + databaseprefix + "article where call_index=@call_index", parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
            }
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(this.column + " from [" + databaseprefix + "article]");
            if (strWhere.Trim() != "")
            {
        ...
        public bool ExistsTitle(string title)
        {
            ...
            return DbHelperSQL.Exists("select count(1) from " + databaseprefix + "article where title=@title", parameters);
        }
        ...
        {
            string title = Convert.ToString(DbHelperSQL.GetSingle("select top 1 title from " + databaseprefix + "article where id=" + id));
        ...
        {
            string content = Convert.ToString(DbHelperSQL.GetSingle("select top 1 content from " + databaseprefix + "article where id=" + id));
        ...
        {
            string str = Convert.ToString(DbHelperSQL.GetSingle("select top 1 click from " + databaseprefix + "article where id=" + id));
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) as H from " + databaseprefix + "article");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public int GetStockQuantity(int id, int goods_id)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select top 1 stock_quantity from " + databaseprefix + "article_goods");
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select top 1 stock_quantity from " + databaseprefix + "article_attribute_value");
        ...
        private Model.article DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                //扩展参数信息
                model.attribute = new article_attribute(databaseprefix).GetList(model.id);
                //商品价格
        ...
        public DataSet GetRandomList(string channel_name, int category_id, int Top, string strWhere)
        {
            ...
            strSql.Append(" * ,newid() as random from view_channel_" + channel_name);
            strSql.Append(" where datediff(d,add_time,getdate())>=0");
            if (category_id > 0)
            {
                strSql.Append(" and category_id in(select id from " + databaseprefix + "article_category where class_list like '%," + category_id + ",%')");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,channel_id,category_id,call_index,title,zhaiyao as remark,tags,img_url,click,site_id,add_time,update_time from " + databaseprefix + "article");
            strSql.Append(" where datediff(d,add_time,getdate())>=0 and status=0");
        ...
        public DataSet GetSearch(string channel_name, int tags_id, int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" id,channel_id,call_index,title,zhaiyao,tags,img_url,click,add_time from [" + databaseprefix + "article] ");
            strSql.Append(" where status=0 and datediff(d,add_time,getdate())>=0 and id in(select article_id from [" + databaseprefix + "article_tags_relation] where tag_id=" + tags_id + ")");
            if (!string.IsNullOrEmpty(channel_name))
            {
                strSql.Append(" and channel_id=(select id from [" + databaseprefix + "channel] where [name]='" + channel_name + "')");
            }
