...
namespace DTcms.DAL
{
	...
	public partial class navigation
	{
        ...
        /// 返回ID
        /// </summary>
        public int GetId(SqlConnection conn, SqlTransaction trans, string name)
        {
            SqlParameter[] parameters = {
					new SqlParameter("@name", SqlDbType.VarChar,50)};
            parameters[0].Value = name;
            object obj = DbHelperSQL.GetSingle(conn, trans, "select id from " + databaseprefix + "navigation where name=@name", parameters);
        ...
        /// 快捷添加系统默认导航，带事务
        /// </summary>
        public int Add(SqlConnection conn, SqlTransaction trans, string parent_name, string nav_name, string title, string link_url, int sort_id, int channel_id, string action_type, int is_lock)
        {
            //先根据名称查询该父ID
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id from " + databaseprefix + "navigation");
        ...
        /// 快捷添加系统默认导航，带事务
        /// </summary>
        public int Add(SqlConnection conn, SqlTransaction trans, int parent_id, string nav_name, string title, string link_url, int sort_id, int channel_id, string action_type, int is_lock)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into " + databaseprefix + "navigation(");
        ...
        {
            DbHelperSQL.ExecuteSql("update [" + databaseprefix + "navigation] set icon_url = replace(icon_url,'/" + oldName + "/','/" + newName + "/')");
        }
        #endregion
